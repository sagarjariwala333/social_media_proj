...
const axiosDataSubmit = data => {
  return async dispatch => {
    try {
      ...
      const response = await DataService.post('/sbo/observationtype/create', data);
...
const axiosDataRead = () => {
  return async dispatch => {
    try {
      ...
      const query = await DataService.get('/sbo/observationtype/getall');
...
const axiosDataSearch = searchItem => {
  return async dispatch => {
    try {
      await dispatch(axiosReadBegin());
      if (searchItem !== '') {
        const query = await DataService.get(`/sbo/observationtype/search/${searchItem}`);
        await dispatch(axiosReadSuccess(query.data.data));
      } else {
        try {
          const query = await DataService.get('/sbo/observationtype/getall');
...
const axiosDataUpdate = (id, data) => {
  return async dispatch => {
    try {
      ...
      await DataService.post(`/sbo/observationtype/update/${id}`, data);
...
const axiosDataDelete = ({ id, getData }) => {
  return async dispatch => {
    try {
      ...
      const data = await DataService.get(`/sbo/observationtype/delete/${id}`);
...
const axiosDataSingle = id => {
  return async dispatch => {
    try {
      ...
      const query = await DataService.get(`/sbo/observationtype/getbyid/${id}`);
