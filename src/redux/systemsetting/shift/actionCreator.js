...
const axiosDataSubmit = data => {
  return async dispatch => {
    try {
      ...
      const response = await DataService.post('/systemsetting/shift/create', data);
...
const axiosDataRead = () => {
  return async dispatch => {
    try {
      ...
      const query = await DataService.get('/systemsetting/shift/getall');
...
const axiosDataSearch = searchItem => {
  return async dispatch => {
    try {
      await dispatch(axiosReadBegin());
      if (searchItem !== '') {
        const query = await DataService.get(`/systemsetting/shift/search/${searchItem}`);
        await dispatch(axiosReadSuccess(query.data.data));
      } else {
        try {
          const query = await DataService.get('/systemsetting/shift/getall');
...
const axiosDataUpdate = (id, data) => {
  return async dispatch => {
    try {
      ...
      await DataService.post(`/systemsetting/shift/update/${id}`, data);
...
const axiosDataDelete = ({ id, getData }) => {
  return async dispatch => {
    try {
      ...
      const data = await DataService.get(`/systemsetting/shift/delete/${id}`);
...
const axiosDataSingle = id => {
  return async dispatch => {
    try {
      ...
      const query = await DataService.get(`/systemsetting/shift/getbyid/${id}`);
