...
const axiosDataSubmit = data => {
  return async dispatch => {
    try {
      ...
      const response = await DataService.post('/systemsetting/employee/create', data);
...
const axiosDataRead = () => {
  return async dispatch => {
    try {
      ...
      const query = await DataService.get('/systemsetting/employee/getall');
...
const axiosDataSearch = searchItem => {
  return async dispatch => {
    try {
      await dispatch(axiosReadBegin());
      if (searchItem !== '') {
        const query = await DataService.get(`/systemsetting/employee/search/${searchItem}`);
        await dispatch(axiosReadSuccess(query.data.data));
      } else {
        try {
          const query = await DataService.get('/systemsetting/employee/getall');
...
const axiosDataUpdate = (id, data) => {
  return async dispatch => {
    try {
      ...
      await DataService.post(`/systemsetting/employee/update/${id}`, data);
...
const axiosDataDelete = ({ id, getData }) => {
  return async dispatch => {
    try {
      ...
      const data = await DataService.get(`/systemsetting/employee/delete/${id}`);
...
const axiosDataSingle = id => {
  return async dispatch => {
    try {
      ...
      const query = await DataService.get(`/systemsetting/employee/getbyid/${id}`);
